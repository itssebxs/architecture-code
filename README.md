...
👉 **[Descargar Documento Técnico en PDF](./docs/Arquitectura_Integracion_Multicore.pdf)** _(Nota: Asegúrate de guardar tu PDF en la carpeta docs/ con este mismo nombre)_
...
1. **Alineación con el Estándar BIAN (Banking Industry Architecture Network):** Aislamiento total de los canales digitales (Banca Web y Móvil) mediante un _Enterprise Integration Layer (EIL)_ organizado por dominios de servicio funcionales (_Customer_, _Product_, y _Payment Execution_).
...
Los componentes lógicos del sistema están estructurados bajo el estándar C4 para garantizar la visibilidad técnica en múltiples niveles. Puedes hacer clic en cada imagen para verla en alta resolución:
...
![Diagrama de Contenedores](./diagrams/02_contenedores.png)
...
![Diagrama de Componentes](./diagrams/03_componentes.png)

---

## 🛡️ Seguridad y Cumplimiento Normativo (LODP)

- **Seguridad Financiera:** Autenticación robusta de canales mediante _Authorization Code con PKCE_ y firmas mediante llaves privadas (_Private Key JWT_) para terceros.
- **Cumplimiento LODP:** Cifrado de datos en reposo y tránsito (AES-256 + mTLS) gestionado por _HashiCorp Vault_. Mecanismos automáticos de **Data Masking** (Enmascaramiento) y anonimización en el flujo de eventos de Kafka para proteger la Información de Identificación Personal (PII) antes de ser consumida por motores de analítica o logs de auditoría.
...
1. **Fase 1:** Despliegue de infraestructura base y coexistencia pasiva (Ruta limpia).
